...
### Testing
We will validate the effectiveness of our model by testing it on unseen Monopoly runs (a new seed that is not in training data). Metrics for success will include correctly predicting winner, average wealth by the end of the game, and the impact of each factor (e.g., property ownership, rent payments, and auction bids) on the game outcome.
